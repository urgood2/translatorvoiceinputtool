...
- **Pinned reproducibility:** model artifact source and revision must be pinned; downloads must be resumable and checksummed.
  ...
  - Added requirement (for implementation readiness): `shared/model/MODEL_MANIFEST.json` schema is explicitly documented (either in the file itself or in `IPC_PROTOCOL_V1.md`), including required fields, allowed hash algorithms (MVP: SHA-256), and exact cache layout expectations.
- **License & attribution compliance:** model + dependencies must permit redistribution for the intended release channel; required notices must ship with the app.
  - Implementation note (testable): add a checked-in third-party notices doc (e.g., `docs/THIRD_PARTY_NOTICES.md`) that lists each redistributed dependency/model artifact with license + source + version/revision; ensure it is referenced from README and included in release artifacts.
  - Implementation note (added for completeness): include a go/no-go check in the M0 model-source spike if licensing/redistribution terms are unclear or incompatible.
  - Added requirement (for clarity): define the MVP “release channel” assumption for licensing checks (e.g., GitHub releases / direct downloads), since obligations differ by channel.
- **No surprise permissions:** clearly prompt for/describe required OS permissions (macOS Microphone + Accessibility; Windows microphone; Linux varies).
  ...
  - Added requirement (for implementation readiness): permissions remediation text is version-controlled (e.g., as string constants) and referenced by stable internal error categories so messaging is consistent and testable.
...
- Python dependencies are pinned (exact versions) in `sidecar/pyproject.toml`; packaging assumes wheels exist for each target OS/arch (if native builds are required, document prerequisites and treat as a packaging risk/spike).
...
- Added constraint (for implementation readiness): avoid any runtime dependency that requires a background daemon/service installation for MVP (keep install/uninstall clean).
...
- **Injection default = clipboard paste** (most reliable for Unicode); “restore clipboard” is configurable.
  ...
  - Added requirement (for testability): define the default `paste_delay_ms` (recommendation: 40ms) and an allowed range; add a unit test that validates config clamping.
...
- Time-bound behavior (testability):
  ...
  - Added defaults (for implementation readiness/testability; configurable later):
    - Default max recording length: 60s (hard cap: 300s).
    - Default “too short” threshold: 250ms (below this, treat as controlled no-op with user-facing message).
    - Default “no transcription event after stop” timeout: 60s (after which transition to error with remediation).
...
│  └─ model/
│     └─ MODEL_MANIFEST.json      # pinned model source/revision/hashes/file list
│
├─ scripts/
...
- Added requirement (for implementation readiness): treat the model manifest as **mandatory for MVP**, owned by M0 (not optional), and add a CI check that the manifest parses and matches the model identifier(s) used in code.
- Added recommendation (for testability/consistency): add `shared/replacements/TEST_VECTORS.json` containing canonical inputs/expected outputs for the replacement+macro pipeline, consumed by both Python tests and UI tests (or Rust tests if UI tests are not added) to prevent semantic drift.
...
- Safety limits (testable):
  ...
  - Added requirement (for implementation readiness): define whether oversized lines are (a) fatal (transition to `error` and require restart) or (b) recoverable (drop line, keep running). Recommendation for MVP: treat as fatal with a clear remediation (“Restart sidecar”).
...
- `asr.initialize` `{ model: "parakeet-v3-0.6b", device_pref: "auto"|"cuda"|"cpu" }` → `{ status:"ready" }`
  ...
  - Added requirement (for implementation readiness): explicitly document which behavior MVP uses. Recommendation for MVP: `asr.initialize` blocks (long timeout) and emits `event.status_changed` progress so the app never appears hung.
...
- `event.status_changed` `{ state, detail? }`
  ...
  - Added requirement (for implementation readiness): define whether `progress.current/total` represent cumulative download bytes vs per-file bytes, and whether `detail` must remain stable enough to display directly in tray/UI.
...
### `ReplacementRule` schema (normative; shared by UI/Rust/Python)
- **Goal:** UI validates and persists the same shape that sidecar executes; protocol examples must include this shape.
- **Type (recommended fields):**
  ...
  - `replacement: string`
  - `word_boundary: boolean` (applies to `kind:"literal"`; if `kind:"regex"`, document whether ignored or translated)
  - `case_sensitive: boolean`
  - `description?: string` (optional, user-facing label)
- **Evaluation order:** list order (top-to-bottom), first-match-wins vs apply-all must be explicitly stated (recommendation: apply-all in order, single pass).
  - Added requirement (for implementation readiness): lock MVP semantics to **apply-all in order, single pass** (no recursion), and add shared test vectors to prevent drift across Python/UI implementations.
- **Constraints (testable defaults; configurable later):**
  - `rules.length` max: 500
  - `pattern` max length: 256
  - `replacement` max length: 1024
  - Total postprocess output max length: 50_000 characters (beyond this, truncate with a clear warning/error category)
...
- Added requirement (for completeness/testability): define canonical timeouts in one place (Rust defaults), referenced by the protocol doc:
  ...
  - Added requirement (for implementation readiness): add a single “timeout policy” section that states which timeout failures are recoverable by retry vs require “Restart sidecar”.
...
- M0.4 Spike: platform capability verification (hotkey hold/release + injection) per OS
  - AC: document observed behavior for Windows, macOS, X11, and Wayland (if available) for (a) global shortcut press/release reliability, (b) paste keystroke injection feasibility, (c) required permissions.
  - AC: for any environment where hold/release is unreliable, specify the default mode (toggle) and the exact UI/tray labeling + Diagnostics wording.
  - AC (added for testability): include a minimal reproducible test harness or manual script that any contributor can run (steps + expected events) and link it from the bd issue.
- M0.5 Spike: model source/revision + manifest + license go/no-go
  - AC: choose a concrete model artifact source (URL/provider), an immutable revision identifier, and produce `shared/model/MODEL_MANIFEST.json` with file list + sizes + hashes.
  - AC: verify redistribution terms for the chosen model artifact and record the conclusion (OK/Not OK/Unknown) with a link to the license/terms in `docs/THIRD_PARTY_NOTICES.md`.
  - AC (added for testability): add a small CI check that (a) manifest parses, (b) hashes are valid hex of expected length, and (c) `model` string used in `asr.initialize` matches the manifest `model_id`.
  - Added requirement (for implementation readiness): the manifest explicitly defines the cache root directory name (e.g., `parakeet-v3-0.6b/<revision>/...`) so both Rust and sidecar can compute expected paths deterministically and tests can assert them.
- M0.6 Spike: sidecar packaging feasibility (audio + ML deps)
  - AC: build a minimal sidecar binary that can run `system.ping` and `audio.list_devices` on at least one target OS end-to-end (local), documenting binary size, startup time, and any native library requirements (e.g., PortAudio).
  - AC: identify blockers for full ASR packaging (torch/Nemo wheels, CUDA bundling, macOS notarization/quarantine, etc.) and file follow-up bd issues with explicit go/no-go criteria.
...
- M1.3 Recorder (push-to-talk) with bounded memory (max seconds; ring buffer/deque) at 16kHz mono float32.
  ...
  - Added requirement (for implementation readiness): choose and document an MVP resampling algorithm (e.g., linear interpolation) and provide a small golden test case (input array → expected resampled output) to ensure determinism across platforms.
- M1.4 Parakeet loader + inference (`asr.initialize`, internal transcribe) with CUDA/CPU fallback.
  ...
  - AC (added for testability): during first-run download/initialize, sidecar emits `event.status_changed` updates (and optional `progress`) so Rust can surface a deterministic “Downloading model…” state rather than appearing hung.
  - AC (added for testability): define and test the “offline after download” invariant: when manifest-validated cache is present, `asr.initialize` and transcription perform no network calls (verified in manual checklist by disabling network).
  - Added requirement (for implementation readiness): explicitly document how the sidecar decides “cache is valid” (all required files exist + sizes match + SHA-256 match) and how it recovers (delete/retry per-file vs full cache purge), and add tests for the “cache validation” function using temp dirs.
- M1.5 Postprocess + replacements pipeline (macros `@@date`, `@@time`, `@@datetime`; snippet word-boundary replacements).
  ...
  - Added requirement (for implementation readiness): lock MVP pipeline order as: (1) postprocess normalization, (2) macro expansion, (3) replacements apply-all in order (single pass, no recursion), and add shared test vectors consumed by Python tests (and UI tests if added).
...
- M2.1 Sidecar manager (`sidecar.rs`): spawn, capture stdout/stderr, restart with backoff and max retries.
  ...
  - Added defaults (for implementation readiness/testability; configurable later):
    - Default max restart attempts: 5.
    - Default backoff: exponential starting at 250ms, capped at 10s.
...
- M2.4 Text injection (`injection.rs`): clipboard paste default + optional restore; fallback to typing when configured.
  ...
  - Added requirement (for implementation readiness): define “restore clipboard” behavior precisely:
    - MVP definition: store pre-injection clipboard, set clipboard to transcript, attempt paste, then restore clipboard best-effort.
    - If restore fails: never roll back injection; always log and include in diagnostics.
    - Add config `restore_clipboard_strict` (default false): if true, surface a non-fatal user-visible warning/toast (but do not transition the global app state to `error`).
...
- M3.3 Replacements manager: CRUD, enable/disable, import/export JSON, preview box (“input → processed output”).
  ...
  - Added requirement (for implementation readiness; resolves ambiguity): for MVP, implement preview via a **local mirror** of the pipeline (postprocess + macros + replacements) in the UI using the **locked semantics** from `IPC_PROTOCOL_V1.md`, and validate it against `shared/replacements/TEST_VECTORS.json` to prevent divergence.
...
- M4.1 Wire hotkey → start/stop → transcription notifications → injection (Rust-owned E2E).
  ...
  - Added requirement (for implementation readiness): define how to enable stub mode (e.g., env var `OPENVOICY_SIDECAR_STUB=1` or alternate bundled stub binary), and ensure it cannot be enabled accidentally in release builds without explicit user action.
...
- M4.3 Logging (Rust + sidecar) with ring-buffer “recent logs” for diagnostics.
  ...
  - Added defaults (for implementation readiness/testability; configurable later): 500 lines or 256 KiB (whichever hits first).
- M4.4 Tests + manual checklist
  ...
  - Added requirement (for implementation readiness): include a deterministic “baseline” test run using sidecar stub mode so contributors can validate hotkey→inject without waiting for model download.
...
- M5.1 Build sidecar binary (PyInstaller) per OS; ensure runtime deps included.
  ...
  - AC (added for completeness/testability): document and validate audio native dependencies required by the capture stack (e.g., PortAudio for `sounddevice`); CI/build scripts must fail with a clear error if these dependencies are missing.
  - AC (added for reproducibility): `sidecar/pyproject.toml` pins exact versions of all runtime deps (torch/Nemo/sounddevice/etc.); build scripts record resolved versions into the build manifest for traceability.
...
- M5.3 CI workflow: build matrix for Windows/macOS/Linux; artifact upload.
  ...
  - Added requirement (for implementation readiness): CI includes a “no-schema-drift” check that compares `ReplacementRule` shape used in UI types and Python validators (even if only by checking required keys in a shared JSON schema/test file).
...
- **License/redistribution constraints:** model and dependency licensing may block distribution.
  - Requirement: model artifact source + license/terms are decided in M0.5; if incompatible/unclear, file a bd issue with options (mirror alternative source, switch artifact, or adjust MVP scope) and a documented go/no-go decision.
  - Added requirement (for completeness): ship third-party notices in `docs/THIRD_PARTY_NOTICES.md` and ensure release artifacts include it.
...
- Added requirement (for completeness): every bd issue that touches packaging/model distribution explicitly calls out licensing/redistribution implications and points at the relevant entry in `docs/THIRD_PARTY_NOTICES.md`.
- Added requirement (for implementation readiness): add a bd issue to lock the MVP “replacement semantics + preview strategy” decision (local mirror + shared test vectors) and ensure it is reflected in `IPC_PROTOCOL_V1.md` and validated in CI.