...
SOURCE_SCRIPT = REPO_ROOT / "scripts" / "e2e" / "test-packaged-app.sh"
...
MOCK_SELF_TEST = textwrap.dedent(
    ...
    print("mock packaged self-test: OK", flush=True)
    """
)


class PackagedAppRuntimeTests(unittest.TestCase):
    target = "x86_64-unknown-linux-gnu"

    def _build_fixture_project(self) -> Path:
        root = Path(tempfile.mkdtemp(prefix="packaged-app-runtime-"))
        (root / "scripts" / "e2e").mkdir(parents=True, exist_ok=True)
        (root / "src-tauri" / "binaries" / "shared" / "replacements").mkdir(parents=True, exist_ok=True)
        (root / "src-tauri" / "binaries" / "shared" / "model" / "manifests").mkdir(parents=True, exist_ok=True)
        (root / "src-tauri" / "binaries" / "shared" / "contracts").mkdir(parents=True, exist_ok=True)
        (root / "src-tauri" / "target" / "release" / "bundle" / "appimage").mkdir(parents=True, exist_ok=True)
        (root / "sidecar" / "src" / "openvoicy_sidecar").mkdir(parents=True, exist_ok=True)

        shutil.copy2(SOURCE_SCRIPT, root / "scripts" / "e2e" / "test-packaged-app.sh")
        ...
        (root / "src-tauri" / "target" / "release" / "bundle" / "appimage" / "mock.AppImage").write_bytes(
            b"mock-appimage"
        )

        shared_root = root / "src-tauri" / "binaries" / "shared"
        (shared_root / "replacements" / "PRESETS.json").write_text("{}", encoding="utf-8")
        (shared_root / "model" / "MODEL_MANIFEST.json").write_text("{}", encoding="utf-8")
        (shared_root / "model" / "MODEL_CATALOG.json").write_text("{}", encoding="utf-8")
        (shared_root / "model" / "manifests" / "fixture.json").write_text("{}", encoding="utf-8")
        (shared_root / "contracts" / "tauri.events.v1.json").write_text("{}", encoding="utf-8")

        (root / "sidecar" / "src" / "openvoicy_sidecar" / "__init__.py").write_text("", encoding="utf-8")
    ...
    def _run_script(
        ...
        script = root / "scripts" / "e2e" / "test-packaged-app.sh"
    ...
    def test_runtime_pass_path_exits_zero_and_validates_resource_paths(self) -> None:
        ...
        try:
            ...
            self.assertIn("system.info resource path validation: OK", completed.stdout + completed.stderr)
            self.assertIn("packaged app smoke test passed", completed.stdout + completed.stderr)
        finally:
    ...
    def test_runtime_windows_target_returns_skip_77(self) -> None:
        ...
        try:
            ...
            self.assertIn("[PACKAGED_APP][SKIP]", completed.stdout + completed.stderr)
        finally:
            shutil.rmtree(root, ignore_errors=True)
